...
const BUYER_TABS: TabItem[] = [
  ...
  { id: 'more', icon: 'Grid', labelKey: 'nav.more', path: '', isMore: true },
];
...
export const BottomTabBar: React.FC = () => {
  ...
  const isActive = (path: string, isMore?: boolean) => {
    if (isMore) {
      // Check if current path is in the "More" menu items
      return MORE_PATHS.some(p => location.pathname === p || location.pathname.startsWith(p + '/'));
    }
    ...
    <>
      ...
      >
        {tabs.map((tab) => {
          ...
          const active = isActive(tab.path, tab.isMore);
